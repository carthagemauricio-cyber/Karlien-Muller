    ...
    <div className="flex flex-col min-h-screen bg-white pb-32">
      <div className="p-4 md:p-8 max-w-2xl mx-auto w-full">
        <div className="mb-10 mt-6 text-center">
            <div className="w-20 h-20 bg-gray-50 text-primary-400 rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl border border-primary-400/20">
                <Calendar size={36} />
            </div>
            <h1 className="text-4xl font-sans text-gray-900 mb-2">{t('my_bookings.title')}</h1>
            <div className="h-1 w-16 bg-primary-400 mx-auto mb-4"></div>
            <p className="text-gray-600 font-light text-sm">{t('my_bookings.subtitle')}</p>
        </div>

        <form onSubmit={handleSearch} className="mb-12">
            <div className="relative flex bg-gray-50 rounded-[28px] overflow-hidden border border-gray-200 shadow-2xl shadow-black/40 group focus-within:border-primary-400/50 transition-all">
                ...
                <input
                    ...
                    className="block w-full pl-16 pr-4 py-5 bg-transparent focus:outline-none text-gray-900 placeholder-gray-400 font-light text-lg"
                />
                <button
                    type="submit"
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
                {myAppointments.length > 0 ? (
                    myAppointments.map(app => {
                        ...
                        return (
                            <div key={app.id} className="bg-gray-50 p-8 rounded-[40px] border border-gray-200 shadow-2xl flex flex-col gap-6 relative overflow-hidden group hover:border-primary-400/30 transition-all">
                                <div className="absolute top-0 right-0 p-8">
                                   {getStatusBadge(app.status)}
                                </div>
                                
                                <div className="pr-20">
                                    <h3 className="font-sans text-2xl text-gray-900 mb-2">{service?.name}</h3>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-2xl bg-white flex items-center justify-center border border-gray-200 text-primary-400">
                                            <Calendar size={18} />
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-0.5">{t('common.date')}</p>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-0.5">{t('common.time')}</p>
                                    ...
                                    <div className="flex items-center gap-4 col-span-1 sm:col-span-2 pt-2">
                                        ...
                                        <div>
                                            <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-0.5">{t('common.professional')}</p>
                                            <span className="text-gray-900 font-medium">{prof?.name}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="text-center py-20 bg-gray-50/50 rounded-[40px] border border-gray-200 border-dashed">
                        <p className="text-gray-600 text-lg font-light">Nenhum agendamento encontrado para este nome.</p>
                    </div>
                )}
            </div>
        )}
