  ...
  return (
    <div className="p-6 md:p-10 max-w-6xl mx-auto pb-24 md:pb-10 relative min-h-screen">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
        <div>
          <h1 className="text-4xl font-sans font-medium text-gray-900 mb-2">{t('management.professionals')}</h1>
          <div className="h-1 w-20 bg-primary-400 mb-2"></div>
          <p className="text-gray-600 font-light">{t('management.professionals_subtitle')}</p>
        </div>
        <button 
        ...
        {professionals.map((prof) => (
          <div key={prof.id} className="bg-gray-50 rounded-[40px] p-8 shadow-2xl border border-gray-200 flex flex-col hover:border-primary-400/30 transition-all group">
            ...
            <div className="space-y-6 flex-1">
              <div>
                <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">{t('management.specialties')}</h4>
...
const ProfessionalModal = ({ prof, onClose, onSave }: { prof: Professional | null, onClose: () => void, onSave: (p: Professional) => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-gray-100/90 backdrop-blur-xl flex items-center justify-center p-4">
      <div className="bg-gray-50 rounded-[40px] p-8 md:p-12 w-full max-w-2xl shadow-2xl border border-gray-200 max-h-[90vh] overflow-y-auto animate-in fade-in slide-in-from-bottom-8 relative">
        <div className="flex justify-between items-center mb-10">
          <div>
            <h2 className="text-3xl font-sans text-gray-900">{prof ? t('management.edit_professional') : t('management.new_professional')}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div>
            ...
            <input 
              ...
              className="w-full px-6 py-4 bg-white border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary-400/30 transition-all font-sans text-xl text-gray-900 placeholder:text-gray-400"
              ...
              <div className="relative group">
                {photo ? (
                  <img 
                    src={photo} 
                    alt="Preview" 
                    className="w-24 h-24 rounded-full object-cover border-2 border-primary-400/20 bg-white shrink-0" 
                  />
                ) : (
                   <div className="w-24 h-24 rounded-full bg-white border-2 border-gray-200 flex items-center justify-center shrink-0">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    specialties.includes(service.id)
                      ...
                      : "bg-white/50 border-gray-200 text-gray-500 hover:border-primary-400/30"
                  )}
                >
                  {service.name}
                </button>
              ))}
              {services.length === 0 && <span className="text-xs text-gray-400 font-light italic">{t('management.add_services_first')}</span>}
            </div>
          </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    days.includes(idx)
                      ...
                      : "bg-white/50 border-gray-200 text-gray-400 hover:border-primary-400/30"
                  )}
                >
                  {day.charAt(0)}
                </button>
