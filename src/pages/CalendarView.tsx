  ...
  return (
    <div className="p-6 md:p-10 max-w-6xl mx-auto pb-24 md:pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          <h1 className="text-4xl font-sans font-medium text-gray-900 mb-2">{t('calendar.title')}</h1>
        ...
        <div className="flex items-center gap-2 bg-gray-50 rounded-full p-1 border border-gray-200 shadow-2xl w-max">
          <button 
            onClick={() => setSelectedDate(prev => addDays(prev, -7))}
            className="p-2 hover:bg-gray-100 rounded-full text-gray-600 hover:text-primary-400 transition-colors"
          >
            <ChevronLeft size={20} />
          </button>
            ...
            <button
              ...
              className={cn(
                ...
                isSelected 
                  ...
                  : "bg-gray-50 border-gray-200 hover:border-primary-400/30 hover:bg-gray-100 text-gray-600"
              )}
            >
              <span className={cn(
      ...
      {/* Timeline View */}
      <div className="bg-gray-50 rounded-[40px] p-6 md:p-10 shadow-2xl border border-gray-200 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-primary-400/30 to-transparent"></div>
        
        <h3 className="text-2xl font-sans text-gray-900 mb-8 flex items-center justify-between">
          ...
          {dayAppointments.length > 0 ? (
            dayAppointments.map((apt) => {
              ...
              const colorClass = apt.status === 'Cancelado' 
                ...
                : 'bg-white border-gray-200 border-l-secondary-400 text-gray-900 hover:border-secondary-400/40';
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-5">
                    <div className="bg-gray-50 p-3.5 rounded-2xl font-mono text-xs font-bold whitespace-nowrap flex items-center gap-2 shadow-inner border border-gray-200 text-primary-400">
                      <Clock size={16} />
                      {apt.time}h
                    </div>
                    <div>
                      <h4 className="font-sans text-xl flex items-center gap-3 text-gray-900">
                        {apt.clientName}
                      ...
                      <p className="text-[10px] font-mono text-gray-500 uppercase tracking-widest mt-1">{apt.clientPhone}</p>
                    </div>
                  </div>
                  
                  <div className="flex items-center gap-10 md:ml-auto pt-4 md:pt-0 border-t md:border-t-0 border-gray-200">
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-[150px]">
                       ...
                       <div>
                         ...
                         <p className="font-medium text-sm text-gray-800">{professional?.name}</p>
                       </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="py-20 text-center bg-white/50 rounded-[32px] border border-gray-200 border-dashed">
              <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 mx-auto mb-6 shadow-inner">
               <CalendarIcon size={36} />
              </div>
              <p className="text-gray-600 text-lg font-light">{t('calendar.no_appointments')}</p>
            </div>
          )}
        </div>
      </div>
